...
export class CheckoutComponent implements OnInit {
    ...
    this.checkoutFormGroup = this.formBuilder.group({
      customer: this.formBuilder.group({
        ...
        email: new FormControl(theEmail, 
            [Validators.required, Validators.pattern('^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,20}$')]        
        )
      }),
  ...
  /*  const startMonth: number = new Date().getMonth() + 1;
    ...
    this.luv2ShopFormService.getCreditCardMonths(startMonth).subscribe(
      data => {
        console.log("Retrieved cred card months: " + JSON.stringify(data, null, 4));
    ...
    this.luv2ShopFormService.getCreditCardYears().subscribe(
      data => {
        console.log("Retrieved cred card years: " + JSON.stringify(data, null, 4));
...
*/
  ...
  onSubmit(){
   ...
   //set up purchase
    ...
    // OLD - call REST API via the CheckoutService
    /*this.checkoutService.placeOrder(purchase).subscribe({
        next: response => {
          alert(`Your order has been received. \nOrder tracking number: ${response.orderTrackingNumber}`)
